...
- `되돌리기` (Ctrl+Z / Command+Z)는 지금 열어 둔 논문에서 새로 만든 표시를 최신순으로 취소합니다. 이전에 저장한 표시는 오른쪽에서 선택해 `하이라이트 삭제`로 지웁니다.
- 드래그 후 메모 입력창이 자동으로 뜨지 않습니다. 오른쪽 문장 목록이나 PDF의 표시를 클릭하면 오른쪽에서 메모를 작성하고 개별 색을 바꾸거나 삭제할 수 있습니다.
- `다시 실행` (Ctrl+Shift+Z / Command+Shift+Z)은 취소한 표시를 메모와 색상까지 복원합니다. 새 표시·메모 편집·색 변경·삭제 시 다시 실행 기록을 초기화합니다.
- 색상 순서는 노랑 → 초록 → 파랑 → 분홍입니다. 취소·복원 안내는 1.2초 동안 표시합니다.
- 메모는 선택 사항이며 약 1.2초 뒤 자동 저장합니다. Enter는 기기에 먼저 보관한 뒤 편집창을 닫고 GitHub 저장을 백그라운드에서 이어갑니다. 상단 저장 상태를 확인할 수 있으며, 연결 실패 시 기기의 초안을 유지해 재시도할 수 있습니다. Shift+Enter는 줄을 바꿉니다. 원본 PDF와 하이라이트 데이터 형식은 그대로 유지합니다.
